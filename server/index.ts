...
function composeAgentPrompt(
  ...
  return [
    ...
    "- Deploy the app to Vercel for preview delivery.",
    "- The Vercel CLI is already installed and already logged in on this machine.",
    "- Do not use localhost, 127.0.0.1, 0.0.0.0, or local:// preview targets.",
    "- Do not rely on a local dev server as the final preview path.",
    "- You must explicitly report the preview target to the preview API whenever it changes.",
    "",
    "Preview API:",
    `- Endpoint: ${previewEndpoint}`,
    `- Bearer token: ${previewToken}`,
    '- After each Vercel deployment, send {"url":"https://your-app.example.com"}.',
    ...
    "- Report the deployed Vercel URL as soon as it exists.",
    "- If you redeploy and the URL changes, call the same API again with the new deployed URL.",
